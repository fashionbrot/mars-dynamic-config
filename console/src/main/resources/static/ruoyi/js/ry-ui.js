...
(function ($) {
    $.extend({
        ...
        treeTable: {
            ...
            init: function(options) {
                ...
                $.bttTable = $('#' + options.id).bootstrapTreeTable({
                    ...
                    type: options.method?options.method:'post',         // 请求方式（*）
                    url: options.url,                                   // 请求后台的URL（*）
    	...
    	modal: {
            ...
            // 弹出层全屏
            openFull2: function (title, url, width, height,btn1,btn2) {
                // 如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
        ...
        operate: {
            ...
            remove: function(id) {
            	...
            	$.modal.confirm("确定删除该条" + table.options.modalName + "信息吗？", function() {
                    ...
                    console.log(table.options.type == table_type.bootstrapTreeTable)
            ...
            openAdd: function(width,height,param){
                var url;
                if (param){
                    url = table.options.createUrl+param;
                }else{
                    url = table.options.createUrl;
                }
                table.set();
                $.modal.open("添加" + table.options.modalName, url ,width,height);
            },
            // 添加信息，以tab页展现
            addTab: function (id) {
            	table.set();
